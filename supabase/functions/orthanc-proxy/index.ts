...
const ORTHANC_AUTH = Deno.env.get('ORTHANC_AUTH');
if (!ORTHANC_AUTH) {
  console.error('[orthanc-proxy] FATAL: ORTHANC_AUTH environment variable is not set.');
}
...
function cleanTokenCache() {
  ...
  for (const [token, exp] of tokenCache.entries()) {
    if (exp < now) {
...
Deno.serve(async req => {
  ...
  try {
    ...
    if (isBypassableRequest(path, req.method)) {
      // Lightweight verification: ensure a valid-looking JWT is passed that hasn't expired.
      const authHeader = req.headers.get('Authorization');
      if (!authHeader) {
        return new Response(
          JSON.stringify({ error: 'Missing Authorization header for bypassed path' }),
      ...
      // Exp check (requires token valid and exp exists and not expired)
      if (!payload || !payload.exp || (payload.exp as number) < Date.now() / 1000) {
        return new Response(
          JSON.stringify({ error: 'Expired or invalid token on bypassable path' }),
          { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      // Cryptographically verify token and cache it for performance
      if (!tokenCache.has(token)) {
        ...
        tokenCache.set(token, payload.exp as number);
        
        // Periodically clean cache to prevent memory leaks
        if (Date.now() - lastCacheCleanup > 60000) {
          cleanTokenCache();
    ...
    if (req.method === 'POST' && path === '/upload-dicom') {
      ...
      if (!orthancResponse.ok) {
        ...
        return new Response(JSON.stringify({ error: `Orthanc upload failed: ${errText}` }), {
          status: 502,
          headers: corsHeaders,
        });
      }
      ...
      const { error: dbError } = await supabase.from('dicom_files').insert({
        user_id: userId,
      ...
      if (dbError) {
        ...
        return new Response(
          JSON.stringify({ error: `DB registration failed: ${dbError.message}` }),
    ...
    if (req.method === 'DELETE') {
      ...
      if (studyMatch) {
        ...
        if (findError) {
          return new Response(JSON.stringify({ error: `DB Query Error: ${findError.message}` }), {
            status: 500,
            headers: corsHeaders,
          });
        }
        ...
        if (!studyData) {
          return new Response(JSON.stringify({ error: 'Study not found or access denied' }), {
            status: 403,
            headers: corsHeaders,
          });
        }
        ...
        if (delFiles.error || delAnn.error || delShared.error) {
          const errMsg =
            delFiles.error?.message || delAnn.error?.message || delShared.error?.message;
          return new Response(JSON.stringify({ error: `DB Deletion Error: ${errMsg}` }), {
            status: 500,
            headers: corsHeaders,
          });
        }
        ...
        return new Response(
          JSON.stringify({ message: 'Study and associated data successfully deleted' }),
          {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          }
    ...
    // ── RESTRICT TO GET FOR ALL OTHER PATHS (except STOW-RS uploads) ─────────
    if (req.method !== 'GET' && !(req.method === 'POST' && path.endsWith('/studies'))) {
    ...
    const fetchOptions: RequestInit = { method: req.method, headers: outHeaders };
    if (req.method === 'POST') {
      fetchOptions.body = await req.arrayBuffer();
    }

    const response = await fetchWithRetry(orthancTarget.toString(), fetchOptions, 20000, 3);
    ...
    const isJsonLike =
      contentType.includes('json') || path.includes('/studies') || path.includes('/metadata');
    ...
    if (response.ok && isJsonLike) {
      try {
        ...
        const orthancIp = Deno.env.get('PUBLIC_VPS_IP') || 'YOUR_VPS_IP';
