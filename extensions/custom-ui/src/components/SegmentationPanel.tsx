...
export function SegmentationPanel({ 
  ...
  useEffect(() => {
    const updateSegmentations = () => {
      ...
      if (reps.length > 0 && reps[0].config?.fillAlpha !== undefined) {
        setGlobalOpacity(reps[0].config.fillAlpha);
      }
    };
    ...
    const subscriptions = [
      segmentationService.subscribe(segmentationService.EVENTS.SEGMENTATION_MODIFIED, updateSegmentations),
      segmentationService.subscribe(segmentationService.EVENTS.SEGMENTATION_REMOVED, updateSegmentations),
      segmentationService.subscribe(segmentationService.EVENTS.SEGMENTATION_DATA_MODIFIED, updateSegmentations),
      segmentationService.subscribe(segmentationService.EVENTS.SEGMENTATION_REPRESENTATION_MODIFIED, updateSegmentations),
    ];
  ...
  }, [activeViewportId, segmentations]);

  const activeSegmentIndex = useMemo(() => {
    if (!activeViewportId || !activeSegmentation) return null;
    const activeSeg = segmentationService.getActiveSegment(activeViewportId);
    return activeSeg?.segmentIndex;
  }, [activeViewportId, activeSegmentation, segmentations]);

  const handleToolClick = (toolName, options = {}) => {
    const toolGroup = toolGroupService.getToolGroupForViewport(activeViewportId);
  ...
  const toggleVisibility = (segmentIndex) => {
    if (!activeSegmentation) return;
  ...
  const handleOpacityChange = (_, value) => {
    ...
    commandsManager.run('setFillAlpha', { value: alpha });
  };

  const getSegmentColor = (segmentIndex) => {
     if (!activeViewportId || !activeSegmentation) return '#3b82f6';
     const color = segmentationService.getSegmentColor(activeViewportId, activeSegmentation.segmentationId, segmentIndex);
  ...
  return (
    <GlassPanel sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 2.5 }}>
      ...
      <Box sx={{ flex: '0 0 auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {toolGroups.map((group, gIdx) => (
          <Box key={gIdx} sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 700, fontSize: '0.6rem', textTransform: 'uppercase', opacity: 0.6 }}>
              {group.title}
            </Typography>
            <Box sx={{ 
              display: 'grid', 
                ...
                return (
                  <Tooltip key={tool.id} title={tool.label} placement="top">
                    <ButtonBase
                      ...
                      onClick={() => handleToolClick(actualToolName, tool.strategy ? { strategy: tool.strategy } : {})}
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <Typography variant="caption" sx={{ color: 'text.secondary', textTransform: 'uppercase', fontWeight: 800, mb: 1.5, fontSize: '0.6rem', opacity: 0.6 }}>
          ...
          <AnimatePresence mode="popLayout">
            {segments.length === 0 ? (
              <Box sx={{ py: 4, textAlign: 'center', opacity: 0.4 }}>
                <Typography variant="caption" color="text.secondary">No segments created</Typography>
              </Box>
            ) : (
                  ...
                  >
                    <Box
                      ...
                      sx={{
                        ...
                        transition: 'all 0.2s',
                        '&:hover': { bgcolor: 'rgba(255, 255, 255, 0.05)', borderColor: isActive ? 'primary.main' : 'rgba(255, 255, 255, 0.1)' },
                      }}
                    >
                      <Box sx={{
                        width: 10,
                      ...
                      <Typography variant="body2" sx={{ flex: 1, fontWeight: 700, color: 'text.primary', fontSize: '0.8rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {segment.label}
                      </Typography>

                      <Stack direction="row" spacing={0} onClick={(e) => e.stopPropagation()}>
                        <Tooltip title={segment.visible !== false ? "Hide" : "Show"}>
                          <IconButton size="small" onClick={() => toggleVisibility(segment.segmentIndex)} sx={{ p: 0.5, color: segment.visible !== false ? 'primary.main' : 'text.disabled', opacity: 0.8 }}>
                            {segment.visible !== false ? <EyeIcon sx={{ fontSize: 16 }} /> : <EyeOffIcon sx={{ fontSize: 16 }} />}
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={segment.locked ? "Unlock" : "Lock"}>
                          <IconButton size="small" onClick={() => toggleLock(segment.segmentIndex)} sx={{ p: 0.5, color: segment.locked ? 'warning.main' : 'text.disabled', opacity: 0.8 }}>
                            {segment.locked ? <LockIcon sx={{ fontSize: 16 }} /> : <UnlockIcon sx={{ fontSize: 16 }} />}
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="Delete">
                          <IconButton size="small" onClick={() => handleDeleteSegment(segment.segmentIndex)} sx={{ p: 0.5, color: 'text.disabled', '&:hover': { color: 'error.main' }, opacity: 0.6 }}>
      ...
      <Box sx={{ mt: 'auto', pt: 2, borderTop: '1px solid rgba(255, 255, 255, 0.05)' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ fontSize: '0.65rem' }}>Global Opacity</Typography>
